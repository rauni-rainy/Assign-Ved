...
exports.pdfWorker = new bullmq_1.Worker('pdf-generation', async (job) => {
    ...
    const paper = await QuestionPaper_1.QuestionPaper.findById(questionPaperId).populate('assignmentId');
    if (!paper) {
        throw new Error(`QuestionPaper with ID ${questionPaperId} not found`);
    }
    try {
        // 2. Call pdfService.generate
        const pdfPath = await pdfService_1.pdfService.generate(paper);
